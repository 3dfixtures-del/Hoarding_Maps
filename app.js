(() => {
  ...
  function pointToPixel(pointNorm, centerNorm, zoom, width, height, pixelScale = 1) {
    const world = 256 * (2 ** zoom) * pixelScale;
  ...
  function pixelToLatlon(x, y, centerNorm, zoom, width, height, pixelScale = 1) {
    const world = 256 * (2 ** zoom) * pixelScale;
  ...
  function verifyProjectionAccuracy(rows, pixels, centerNorm, zoom, width, height, pixelScale = 1) {
    const failures = [];
    rows.forEach((row, i) => {
      const [lat2, lon2] = pixelToLatlon(pixels[i][0], pixels[i][1], centerNorm, zoom, width, height, pixelScale);
  ...
  async function blobToBitmap(blob) {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        URL.revokeObjectURL(url);
        reject(new Error('Google map image could not be decoded.'));
      };
      img.src = url;
    });
  }

  async function fetchGoogleStaticMap(centerNorm, zoom, logicalWidth, logicalHeight, scale, mapType, apiKey) {
    const [lat, lon] = normToLatlon(centerNorm[0], centerNorm[1]);
    const cacheKey = [lat.toFixed(7), lon.toFixed(7), zoom, logicalWidth, logicalHeight, scale, mapType].join('|');
    if (state.googleMapCache.has(cacheKey) && !apiKey) return state.googleMapCache.get(cacheKey);

    const requestPromise = (async () => {
      const response = await fetch('/.netlify/functions/google-static-map', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        // A normal static local server (Live Server, python http.server, etc.)
        // cannot execute Netlify Functions. Give a direct fix instead of a vague 405.
        if (response.status === 405 && isLocalHost) {
          throw new Error(
            'Local server is not running the Netlify Function. Run npm install, then npm run dev, ' +
            'and open http://localhost:8888. Do not use VS Code Live Server for Google map generation.'
          );
        }

        let message = `Google Maps request failed (${response.status}).`;
    ...
    if (!apiKey) state.googleMapCache.set(cacheKey, requestPromise);
    try {
      return await requestPromise;
    } catch (error) {
      if (!apiKey) state.googleMapCache.delete(cacheKey);
      throw error;
    }
  }

  function getGoogleMapPane(width, height) {
    const { x0: tableX0 } = tableGeometry(width);
    const gap = Math.max(12, Math.round(width * 0.012));
    // Google scale=2 gives exactly 2 output pixels per logical Static Maps pixel.
    const mapWidth = Math.floor((tableX0 - gap) / 2) * 2;
    const mapHeight = Math.floor(height / 2) * 2;
    return { mapWidth, mapHeight, gap, tableX0 };
  }
  ...
  async function renderMap(rows, width, height, highlightLabels) {
    ...
    // Neutral water-like presentation background behind the separate table card.
    ctx.fillStyle = '#ABD5E3';
    ctx.fillRect(0, 0, width, height);

    const { mapWidth, mapHeight, tableX0 } = getGoogleMapPane(width, height);
    const scale = 2;
    const logicalWidth = Math.floor(mapWidth / scale);
    const logicalHeight = Math.floor(mapHeight / scale);
    if (logicalWidth > 640 || logicalHeight > 640) {
      throw new Error(
        `This output needs a Google Static Maps image larger than the standard 640×640 logical-pixel limit. ` +
        `Use 1920×1080, 1600×900, or 1280×720.`
      );
    }

    const pointNorms = rows.map((row) => latlonToNorm(row.Lat, row.Long));
    // Static Maps coverage uses logical pixels; the scale=2 image only doubles resolution.
    const safeWLogical = logicalWidth * 0.68;
    const safeHLogical = logicalHeight * 0.76;
    const zoom = chooseZoom(pointNorms, safeWLogical, safeHLogical);
    const center = chooseCenter(pointNorms, zoom, logicalWidth, logicalHeight, logicalWidth * 0.50, logicalHeight * 0.51);

    renderInfo.textContent = 'Loading Google Map…';
    const baseMap = await fetchGoogleStaticMap(
      center,
      zoom,
      logicalWidth,
      logicalHeight,
    ...
    // Preserve Google's original aspect ratio and all built-in logo/attribution.
    ctx.drawImage(baseMap, 0, 0, mapWidth, mapHeight);

    const markerRadius = Math.max(13, Math.round(height * 0.0165));
    const markerPixels = pointNorms.map((p) => pointToPixel(p, center, zoom, mapWidth, mapHeight, scale));
    verifyProjectionAccuracy(rows, markerPixels, center, zoom, mapWidth, mapHeight, scale);

    const mapRight = Math.min(mapWidth - Math.max(8, Math.round(width * 0.006)), tableX0 - Math.max(10, Math.round(width * 0.012)));
    const bubblePixels = placeCalloutBubbles(markerPixels, mapWidth, mapHeight, mapRight, markerRadius);
  ...
  async function generate() {
    ...
    downloadBtn.classList.add('hidden');
    showStatus('Generating Google map. GPS anchors are projection-checked before the callouts are drawn.', 'info');
    ...
    try {
      ...
      renderInfo.textContent = `${width} × ${height} • Google Maps • Exact GPS callouts`;
      showStatus(`Google map generated successfully at ${width} × ${height}.`, 'success');
    } catch (error) {
      console.error(error);
      showStatus(
        `${error.message || String(error)}\n\nCheck that Maps Static API is enabled, the API key/billing/quota are valid, and GOOGLE_MAPS_API_KEY is configured in Netlify (or enter a testing key above).`,
        'error'
      );
      renderInfo.textContent = 'Generation failed.';
    } finally {
      generateBtn.disabled = false;
      generateBtn.textContent = 'Generate Google GPS map';
    }
  }
  ...
  function downloadPng() {
    ...
    a.href = url;
    a.download = `${base}_google_site_location_map.png`;
